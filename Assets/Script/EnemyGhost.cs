    ...
    void MovementsandFlip()
    {
        if(PlayerTransform != null)
        {
            ...
            if(distance < radius)//距離小於設定，啟動跟隨
            {
                ...
                transform.position = Vector2.MoveTowards(transform.position, PlayerTransform.position, speed * Time.deltaTime);//朝玩家移動
            ...
            else//巡邏模式
            {
                ...
                transform.position = Vector2.MoveTowards(transform.position, MovePos.position, speed * Time.deltaTime);//鬼魂移動方式
